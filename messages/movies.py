import random
import requests
import json

from dotenv import load_dotenv
import os


### http://www.omdbapi.com/
### https://imdb-api.com/api
def get_popular_movie():
    with open("data/popular_movies.json", "r") as f:
        movies = json.load(f)
    movie = random.choice(movies["items"])
    movie_id = movie.get("id")

    title = movie.get("fullTitle")
    plot, poster_url = get_movie_plot_and_poster(movie_id)
    str = title + "\n\n" + plot
    return (str, poster_url)


def get_movie_plot_and_poster(imdb_id: str):
    key = os.environ.get("MOVIE_KEY")
    url = "https://www.omdbapi.com/?i={}&apikey={}".format(imdb_id, key)
    response = requests.get(url).text
    data = json.loads(response)
    plot = data.get("Plot")
    poster = data.get("Poster")
    return plot, poster


def get_top_movie():
    with open("data/250_movies.json", "r") as f:
        movies = json.load(f)
    movie = random.choice(movies["items"])
    movie_id = movie.get("id")

    title = movie.get("fullTitle")
    plot, poster_url = get_movie_plot_and_poster(movie_id)
    str = title + "\n\n" + plot
    return (str, poster_url)


def get_k_drama():
    with open("data/k_dramas.json", "r") as f:
        dramas = json.load(f)
    drama = random.choice(dramas["items"])
    drama_id = drama.get("id")

    title = drama.get("fullTitle")
    plot, poster_url = get_movie_plot_and_poster(drama_id)
    str = title + "\n\n" + plot
    return (str, poster_url)
